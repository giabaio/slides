...
These are the data that define the specific presentation such as: 
   ...
   - `thank-you` (a multi-item field with values `show` and the name of the `file` containing the thank-you `gif`. If `show` is `true` then shows the thank-you slide at the end of the presentation, according to the value of `file`, whose options are `reese`, `phoebe`, `michelle`, `ted` and `joey` --- can add as many as needed...). If a `gif` file is too large, can use instead `MP4` format -- and can use the file `gif2mp4` to convert.
