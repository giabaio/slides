---
#' Format. Here selects the style of the slides; other choice is 'gb-revealjs'.
#' Can also output to beamer with ucl-beamer and that creates a native pdf presentation
format: 
  ucl-revealjs
...
- The default output is `html` via `revealjs`. This is managed by the option 
  ```yaml
  format: 
     ucl-revealjs
  ```
  in the `yml` of the `.qmd` file.

- Alternatively, the slides can be produced in `pdf` directly via `beamer`. This is obtained by specifying 
  ```yaml
  format: 
     ucl-beamer
  ```
...
Here's a slide with no logo -- just add `{.no-logo}` to the title of the slide

## Embedding sites

- Can also embed websites as `iframes`, using the code
````
## {background-iframe="https://gianluca.statistica.it" background-interactive="true" data-preload="true" .no-logo}
````

- This creates a title-less slide with the full space occupied by the website.
   - Use the options `background-interactive="true"` and `data-preload="true"` for best control and maximum load speed
...
To use colours with LaTeX, needs a little hack: `$\class{color}{expression}$` 
...
These use the predefined LaTeX macros, saved in the file [`latex_macros.html`](latex_macros.html). More can be (re)defined by simply adding to the file, which gets pre-loaded at the beginning of the resulting `.html` output; example: `$\DIC=2345$` is typeset as $\DIC=2345$.
...
:::{.callout-important}
The font size in code chunks is controlled by the default `css` style. This can be increase by modifying the file `./css/common.css` by changing the value for `font-size`
```
...
Using the tag `{.divider}` makes for some nicer `css` formatting (especially under `ucl-revealjs`) 

:::: {.content-visible when-format="revealjs"}
# And another {.divider-bg footer="false"}

Using the tag `{.divider-bg}` gives a background in line with the UCL 2026 template

The slide looks nicer without the footer. This can be done either by adding
`::: {.footer style="background-color: transparent!important; background: transparent!important"}`    
`:::`    
to the end of the slide, to ensure that the footer doesn't interfere with the colours...

More simply, can explicitly use the tag `{.divider-bg footer="false"}`

# Section title slide {background-image="https://www.ianvisits.co.uk/articles/wp-content/uploads/2022/11/ucl-observatory-mill-hill-01.jpg" background="#002248" background-opacity=0.5 footer="false"}
...
::: {.callout-note}
By default, `ggplot` uses a specialised `theme_ucl()` function (defined in `.assets/setup.R`) to format the graphs. This aligns with the background colour of the slides.
:::
...
The data were extracted from the 1974 Motor Trend US magazine and comprise fuel consumption and 10 aspects of automobile design and performance for 32 automobiles.
:::
...
::: {.column width="62%"}
```{r}
head(mtcars)[,c("mpg",	"cyl", "disp", "hp", "wt")] |> tinytable::tt()
```
:::
:::

::: {style="margin-top: 50px;"}
**NB**: Can use the package `tinytable` to format tables in a very efficient way...
:::
...
- The file `beamer-meta.lua` defines a filter to map the code `::: {.fragment .highlight-XXX}`, which is specific to `revealjs`
-  More colours can be defined and added to the filter, in the part
   ...
   \end{verbatim}
   ```
   and the `new-colour` must also be defined in the `ucl-beamer.tex` template (both under `_extensions/ucl/beamer`)
::::

## Frame-Controlled Fragments

- Some initial bullet point
- And another
   - Possibly nested...

:::: {style="display: grid;"}
::: {.fragment .fade-out fragment-index=3 style="grid-area: 1/1;"}

::: {.fragment .fade-in fragment-index=1  style="grid-area: 1/1; margin-top: -20px;"}
- Some text here
   - And some other
:::

::: {.fragment .current-visible fragment-index=2 style="grid-area: 1/1; margin-top: -20px;"}
- Some more text
:::
:::

::: {.fragment .fade-in fragment-index=3 style="grid-area: 1/1; margin-top: -20px;"}
- More still
- And that's it
:::
::::
...
:::: {.content-visible when-format="revealjs"}
## Slide Backgrounds (only in `revealjs`) {background="#43464B"}
...
## Media Backgrounds (only in `revealjs`)  {background="#43464B" background-image="https://www.ianvisits.co.uk/articles/wp-content/uploads/2022/11/ucl-observatory-mill-hill-01.jpg"}
...
## Media Backgrounds with brand colour (only in `revealjs`) {background="#002248" background-image="https://www.ianvisits.co.uk/articles/wp-content/uploads/2022/11/ucl-observatory-mill-hill-01.jpg" background-opacity="0.5"}
...
## Tabsets {.no-logo transition="slide" .smaller}
...
::: {style="height: 60%!important; overflow: auto; font-size: .63em;"}
```{r}
knitr::kable(mtcars)
```
:::
